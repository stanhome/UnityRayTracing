...
public class MotionBlurSkyboxTutorial : RayTracingTutorial
{
	private FocusCamera _focusCamera = null;

	public MotionBlurSkyboxTutorial(RayTracingTutorialAsset asset) : base(asset)
	{
	}
	...
	public override void render(ScriptableRenderContext context, Camera c)
	{
		...
		try
		{
			using (new ProfilingScope(cmd, new ProfilingSampler("RayTracing")))
			{
				...
				cmd.SetRayTracingTextureParam(_asset.shader, SID_skybox, _asset.skybox);
