		...
		Pass {
			...
			void closestHitShader(inout RayIntersection rayIntersection: SV_RayPayload, AttributeData attributeData: SV_IntersectionAttributes) {
				...
				if (rayIntersection.remainingDepth > 0) {
					...
					float3 reflectDir = reflect(direction, normalInWorld);

					RayDesc rayDescriptor;
					rayDescriptor.Origin = posInWorld + 0.001f * reflectDir;
					// direction = normalize(pos + N + randomOnSphere - pos)
					rayDescriptor.Direction = normalize(reflectDir + _Fuzz * getRandomOnUnitSphere(rayIntersection.PRNGStates));
